...
def PMID_reference(args):
    ...
    publication_dict = webio.build_pub_dict_from_PMID(PMID_list, config_dict["PubMed_search"]["PubMed_email"])
...
#    matching_key_for_citation = [key if key else Google_Scholar_matching_key_for_citation[count] for count, key in enumerate(matching_key_for_citation)]
    ...
    fileio.save_reference_summary_report_to_file(publication_dict, matching_key_for_citation, is_citation_in_prev_pubs_list, reference_lines, tokenized_citations, save_dir_name)
...
#    config_dict = helper_functions.overwrite_config_with_CLI(config_dict, args)
    ...
    print("Searching PubMed.")
    PubMed_publication_dict = webio.search_PubMed_for_pubs(prev_pubs, config_dict["Authors"], config_dict["PubMed_search"]["PubMed_email"], args["--verbose"])
    prev_pubs.update(PubMed_publication_dict)
    print("Searching ORCID.")
    ORCID_publication_dict = webio.search_ORCID_for_pubs(prev_pubs, config_dict["ORCID_search"]["ORCID_key"], config_dict["ORCID_search"]["ORCID_secret"], config_dict["Authors"], args["--verbose"])
    prev_pubs.update(ORCID_publication_dict)
    print("Searching Google Scholar.")
    Google_Scholar_publication_dict = webio.search_Google_Scholar_for_pubs(prev_pubs, config_dict["Authors"], config_dict["Crossref_search"]["mailto_email"], args["--verbose"])
    prev_pubs.update(Google_Scholar_publication_dict)
    print("Searching Crossref.")
    Crossref_publication_dict = webio.search_Crossref_for_pubs(prev_pubs, config_dict["Authors"], config_dict["Crossref_search"]["mailto_email"], args["--verbose"])
    ...
    fileio.save_project_reports_to_file(publication_dict, save_dir_name, config_dict["project_descriptions"])
