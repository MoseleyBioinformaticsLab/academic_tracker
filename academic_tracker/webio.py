...
def search_PubMed_for_pubs(prev_pubs, authors_json_file, authors_by_project_dict, from_email, verbose):
    """Searhes PubMed for publications by each author.
    
    For each author in the projects of authors_by_project_dict PubMed is queried for the publications. The list of publications is then filtered 
    ...
    Args:
        ...
        authors_by_project_dict (dict): keys are project names and values should be a dict of author names with attributes
    ...
    ########################
    for project, author_dict in authors_by_project_dict.items():
        for author, author_attributes in author_dict.items():
            ...
            for pub in publications:
                ...
                if publication_date < author_attributes["cutoff_year"] or \
                   helper_functions.is_pub_in_publication_dict(pub_id, prev_pubs, title_classifier) or \
                   helper_functions.is_pub_in_publication_dict(pub_id, publication_dict, helper_functions.classify_publication_dict_titles(publication_dict)):
                    continue
...
def search_ORCID_for_pubs(prev_pubs, ORCID_key, ORCID_secret, authors_by_project_dict, verbose):
    """Searhes ORCID for publications by each author.
    
    For each author in the projects of authors_by_project_dict ORCID is queried for the publications. The list of publications is then filtered 
    by prev_pubs, affiliations, and cutoff_year. If the publication is in the list of prev_pubs then it is skipped. 
    If the author doesn't have at least one matching affiliation then the publication is skipped. If the publication 
    was published before the cutoff_year then it is skipped. Each publication is then determined to have citations 
    for any of the grants in the author's grants.
    ...
    for project, author_dict in authors_by_project_dict.items():
        for author, authors_attributes in author_dict.items():
...
def search_Google_Scholar_for_pubs(prev_pubs, authors_by_project_dict, mailto_email, verbose):
    """Searhes Google Scholar for publications by each author.
    
    For each author in the projects of authors_by_project_dict Google Scholar is queried for the publications. The list of publications is then filtered 
    by prev_pubs, affiliations, and cutoff_year. If the publication is in the list of prev_pubs then it is skipped. 
    If the author doesn't have at least one matching affiliation then the publication is skipped. If the publication 
    was published before the cutoff_year then it is skipped. Each publication is then determined to have citations 
    for any of the grants in the author's grants.
...
def search_Crossref_for_pubs(prev_pubs, authors_json_file, authors_by_project_dict, mailto_email, verbose):
    """Searhes Crossref for publications by each author.
    
    For each author in the projects of authors_by_project_dict Crossref is queried for the publications. The list of publications is then filtered 
    by prev_pubs, affiliations, and cutoff_year. If the publication is in the list of prev_pubs then it is skipped. 
    If the author doesn't have at least one matching affiliation then the publication is skipped. If the publication 
    was published before the cutoff_year then it is skipped. Each publication is then determined to have citations 
    for any of the grants in the author's grants.
